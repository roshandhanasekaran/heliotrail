...
export const ANOMALY_SEVERITY: Record<MLAnomaly["severity"], string> = {
  ...
  medium: "bg-[var(--passport-amber-muted)] text-[#92400E]",
...
export const PATTERN_BADGE: Record<MLAnomaly["pattern"], { label: string; color: string }> = {
  ...
  "one-off": { label: "One-off", color: "text-muted-foreground" },
};
  ...
  return (
    <div className="px-2.5 py-2.5 space-y-1">
      ...
      <p className="text-[11px] font-semibold leading-tight text-foreground">
        {anomaly.type}
        {anomaly.module && (
          <span className="ml-1 font-mono text-[9px] font-normal text-muted-foreground/70">
            {anomaly.module}
          </span>
        )}
      </p>
      ...
      <span className="text-[8px] text-muted-foreground/70">{anomaly.timestamp}</span>
    </div>
  );
}
